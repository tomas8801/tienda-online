...
//                . "(SELECT producto_id FROM lineas_pedidos WHERE pedido_id={$id})";
    ...
    public function save() {
        $sql = "INSERT INTO pedidos VALUES(null, :usuario_id, :producto_id, :unidades, :provincia, :localidad, :direccion, :cod_postal, :telefono, 'confirm', CURDATE(), CURTIME())";
        $connect = $this->db;
        $stmt = $this->db->prepare($sql);
        $stmt->execute(array(
        ...
        $_SESSION['pedido_id'] = $connect->lastInsertId();
        $result = false;
        if ($stmt) {
            $result = true;
        }
    ...
    public function unidades($unidades = null) {
        if(!is_null($unidades)) {
